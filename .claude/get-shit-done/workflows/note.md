...
- **Global scope**: `/home/skogix/small-hours/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/skogix/small-hours/.claude/notes/`)
...
2. Glob `/home/skogix/small-hours/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/skogix/small-hours/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/skogix/small-hours/.claude/notes/` — works in any directory
